...
# Change PYTHONPATH to include olecfrc so that we can get the version.
sys.path.insert(0, '.')

import olecfrc  # pylint: disable=wrong-import-position
      ...
      for line in iter(spec_file):
        if line.startswith('Summary: '):
          ...
          lines.extend([
              ...
              '%{python3_sitelib}/olecfrc/*.yaml',
              '%{python3_sitelib}/olecfrc*.egg-info/*',
              '',
              '%exclude %{_prefix}/share/doc/*',
              '%exclude %{python3_sitelib}/olecfrc/__pycache__/*',
              '%exclude %{_bindir}/*.py'])
          ...
          python_spec_file.extend([
              ...
              f'Requires: {python_package:s}-olecf-kb >= %{{version}}',
...
olecfrc_long_description = (
    'olecfrc is a Python module part of olecf-kb to allow reuse of Object '
    'Linking and Embedding (OLE) Compound File resources.')
